...
class NachrichtenAtSpider(FeedsXMLFeedSpider):
    ...
    _icon = 'http://static1.{}.at/oonup/images/apple-touch-icon.png'.format(
        name)
    ...
    def start_requests(self):
        ...
        if self._ressorts:
            ...
            self.logger.info(
                'No ressorts given, falling back to general ressort!')
        ...
        if username and password:
            yield scrapy.FormRequest(
                ...
                formdata={'user[control][login]': 'true',
                          'username': username,
                          'password': password},
    ...
    def feed_headers(self):
        for ressort in self._ressorts:
            yield self.generate_feed_header(
                title='{} {}'.format(self._title, ressort.title()),
                path=ressort)
    ...
    def _after_login(self, response=None):
        ...
        for ressort in self._ressorts:
            yield scrapy.Request(
                'https://www.{}/storage/rss/rss/{}.xml'.format(
                    self.name, ressort), meta={'ressort': ressort,
                                               'dont_cache': True})

    def parse_node(self, response, node):
        url = node.xpath('link/text()').extract_first()
        yield scrapy.Request(
            url.replace('#ref=rss', ',PRINT'), self._parse_article,
