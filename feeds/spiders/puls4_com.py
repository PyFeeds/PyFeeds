...
class Pusl4ComSpider(FeedsSpider):
    ...
    _icon = ('http://www.puls4.com/bundles/wundermanpuls4/images/' +
             'favicon/favicon.png')

    def parse(self, response):
        path = json.loads(response.text)['content'][0]['url']
        yield scrapy.Request(response.urljoin(path), self._parse_shows_list,
    ...
    def _parse_shows_list(self, response):
        ...
        for show in shows:
            ...
            yield scrapy.Request(
                ...
                self._parse_show, meta={'dont_cache': True,
                                        'time': time,
